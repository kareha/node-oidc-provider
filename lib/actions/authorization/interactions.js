...
module.exports = async function interactions(resumeRouteName, ctx, next) {
  ...
  if (!prompt) {
    ...
    if (!oidc.session.accountId()) {
      throw new errors.AccessDenied('authorization request resolved without requesting interactions but no account id was resolved', undefined);
    }
    ...
    if (!oidc.acceptedScope()) {
      throw new errors.AccessDenied('authorization request resolved without requesting interactions but no scope was granted', undefined);
    }
